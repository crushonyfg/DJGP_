...
active_jgp_dir = os.environ.get("ACTIVE_JGP_DIR")
if active_jgp_dir and active_jgp_dir not in sys.path:
    sys.path.append(active_jgp_dir)
...
def run_al4jgp(seed=123,
    ...
    if ActiveLearner is None:
        raise ImportError(
            "ActiveLearner is not importable. Set ACTIVE_JGP_DIR to the external "
            "ActiveJGP checkout or add it to PYTHONPATH before running run_al4jgp()."
        )
