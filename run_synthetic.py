...
def _run_djgp(ds, seed, cfg, device):
...
def _run_baselines(ds, seed, preset, device, methods):
    _set_seed(seed)
    ...
    if "DeepGP" in methods:
        if not _DGP_OK:
            ...
            try:
                rows.append(_run_dgp(ds, seed, Xtr, ytr, Xte, yte, device))
...
def _print_table(rows):
    ...
    print(f"\n{'Dataset':8s}  {'Method':12s}  {'RMSE':>16}  {'CRPS':>16}  {'cov90':>10}")
    print("-" * 68)
    for ds in ("L2", "LH"):
        for mth in ("DJGP", "XGBoost", "DKL-SVGP", "DeepGP"):
            ...
            print(f"{ds:8s}  {mth:12s}  {ms('rmse'):>16}  {ms('crps'):>16}  {ms('cov90'):>10}")
...
def main():
    ...
    for ds in datasets:
        cfg = _CONFIGS[ds]
        preset = dict(SETTING_PRESETS[cfg["setting"]])
        for sd in seeds:
            print(f"\n[{ds}  seed={sd}]", flush=True)
            if "DJGP" in methods:
                try:
                    r = _run_djgp(ds, sd, cfg, device)
                    if r:
                        allrows.append(r)
            ...
            if bl:
                try:
                    for r in _run_baselines(ds, sd, preset, device, bl):
                        allrows.append(r)
